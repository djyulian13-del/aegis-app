...
async function broadcast(request, env){ try{ const u=new URL(request.url); const key=u.searchParams.get('key')||request.headers.get('x-admin-key')||''; if(!env.ADMIN_KEY||key!==env.ADMIN_KEY) return json({ok:false,error:'unauthorized'},401); let b; try{ b=await request.json(); }catch(e){ return json({ok:false,error:'bad body'},400); } const subject=(b.subject||'').toString(); const html=(b.html||'').toString(); if(!subject||!html||!env.RESEND_API_KEY) return json({ok:false,error:'missing'},400); const fromAddr=env.RESEND_FROM||'PJ ALERT <aegis@elartedelproteger.com>'; const listing=await env.AEGIS_SOS.list({prefix:'sub:'}); if(u.searchParams.get('dry')) return json({ok:true,dry:true,total:listing.keys.length}); let sent=0,failed=0; for(const k of listing.keys){ const email=k.name.slice(4); try{ const r=await fetch('https://api.resend.com/emails',{method:'POST',headers:{'Authorization':'Bearer '+env.RESEND_API_KEY,'Content-Type':'application/json'},body:JSON.stringify({from:fromAddr,to:[email],subject:subject,html:html+unsubFooter(email),headers:{'List-Unsubscribe':'<'+unsubUrl(email)+'>','List-Unsubscribe-Post':'List-Unsubscribe=One-Click'}})}); if(r.ok)sent++;else failed++; }catch(e){ failed++; } } return json({ok:true,total:listing.keys.length,sent,failed}); }catch(__e){ return json({ok:false,error:String((__e&&__e.stack)||__e)}); } } async function adminStats(request, env){
